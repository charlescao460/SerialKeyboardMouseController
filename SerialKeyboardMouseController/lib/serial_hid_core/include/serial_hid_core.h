 ...
 * @brief C core that parses fixed protocol frames and dispatches HID actions.
 *
 ...
 *
 * Length:
 *   Number of bytes in <Data...> + <Checksum>.
 *
 * Checksum:
 *   Checksum bytes generated by the configured shd_checksum_t callbacks.
 *   Current default usage in this project is one-byte XOR checksum.
 *
 * Data payload format by command:
 ...
 *   Key release all:
 *     AB 03 BC 00 BC
 *   Rel move (+5, -3) with XOR checksum:
 *     AB 06 A0 05 00 FD FF A7
 */
...
/**
 * @brief Processes at most one frame from serial input.
 *
 * On successful command execution, the original frame is always sent back as ACK.
 */
